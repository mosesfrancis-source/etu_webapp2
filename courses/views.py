...
from .forms import AssignLecturerForm, CourseForm, GradeForm
...
def assign_lecturer(request, course_id):
    course = get_object_or_404(Course, id=course_id)
    if request.method == 'POST':
        form = AssignLecturerForm(request.POST, instance=course)
        if form.is_valid():
            form.save()
            lecturer = form.cleaned_data['lecturer']
            if lecturer:
                messages.success(request, f"Assigned {lecturer.get_full_name()} to {course.code}.")
            else:
                messages.success(request, f"{course.code} is now unassigned.")
            return redirect('manage_courses')
    else:
        form = AssignLecturerForm(instance=course)
    return render(request, 'courses/assign_lecturer.html', {'form': form, 'course': course})
